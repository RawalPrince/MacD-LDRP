...
export default function InKitchen({ Products, handleChangesInStatus }) {
    ...
    useEffect(() => {
        ...
        Products.forEach((order) => {
            ...
            order.OrderItem.forEach((item) => {
                initialToggles[order.id][item.product ? item.product.id : item.meal.id] = false;
            });
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-2 border rounded-lg p-2 overflow-y-auto min-h-[130px] max-h-[150px]">
                                {order.OrderItem.map((item, index) => {
                                    const itemId = item.product ? item.product.id : item.meal.id;
                                    return (
                                        <div
                                            key={index}
                                        ...
                                        >
                                            ...
                                            <div className="w-12 h-12 bg-gray-200 rounded-md flex items-center justify-center">
                                                <img
                                                    src={item.product ? item.product.image : item.meal.image}
                                                    alt={item.product ? item.product.name : item.meal.name}
                                            ...
                                            <div className="flex-1">
                                                <div className="font-semibold text-[14.8px]">
                                                    {item.product ? item.product.name : item.meal.name}
                                                </div>
                                            ...
                                            <div
                                                className={`w-[50px] h-[30px] flex items-center cursor-pointer ${toggled[order.id] &&
                                                    toggled[order.id][itemId]
                                                    ? "bg-green-500"
                                                    : "bg-gray-300"
                                                    } rounded-full p-1`}
                                                onClick={() => handleToggle(order.id, itemId)}
                                            >
                                                <div
